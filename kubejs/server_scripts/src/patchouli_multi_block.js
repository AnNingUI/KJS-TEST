    ...
    */
    ...
    if (block.id === 'minecraft:sculk_shrieker') {
        ...
        let Amountreduce = () => {
            ...
            let ftBlockCap = /**@type { Internal.IFluidHandler_ }*/ (ftBlockBe.getCapability(ForgeCapabilities.FLUID_HANDLER).orElse(null))
 ...
 *
 * 根据可用的物品选择最合适的配方。
 * @param {Internal.ItemStack[]} ItemList - 可用物品的列表。
 * @param {{ input: Internal.ItemStack[], Amount: number, output: Internal.ItemStack[] }[]} recipes - 配方列表。
 * @returns {{ input: Internal.ItemStack[], Amount: number, output: Internal.ItemStack[] } | null} - 选择的配方或如果没有合适的配方则返回 null。
 */
